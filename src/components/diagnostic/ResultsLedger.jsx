...
function buildInferredObservation(summary, inputData) {
  ...
  // Media + within leadership + no forum
  ...
  // Tech + startup + external prior attempt
  ...
  // No high-signal match — return null, section won't render
  return null;
}
...
export default function ResultsLedger({ summary, dispatchUrl, onReset, inputData }) {
  ...
  if (el) {
    ...
    // Force ALL elements to dark text and transparent backgrounds
    // for print legibility regardless of theme token values
    ...
    // Re-apply accent color to elements that should stay orange
    el.querySelectorAll('[class*="text-brand-accent"]').forEach(node => {
      node.style.color = '#FF4500';
    });

    // Re-apply background to the container itself
    el.style.background = '#FAF9F6';
  ...
  return (
    <div className="space-y-10 w-full">
      ...
      <div
        ...
        className="bg-brand-bg border border-brand-border p-10 lg:p-16 shadow-2xl relative transition-colors duration-700"
      >
        {/* Watermark */}
        ...
        <div className="space-y-14">
          ...
          <div className="space-y-5">
            ...
            <p
              className="font-serif italic text-brand-muted border-l-4 border-brand-accent pl-8 py-2 max-w-3xl leading-relaxed"
          ...
          <div className="space-y-5">
            ...
            <motion.p
              ...
              className="font-serif italic text-brand-text leading-relaxed max-w-3xl border-l-2 border-brand-accent/40 pl-8"
...
<div className="border-t-2 border-brand-accent p-10 space-y-8 bg-brand-bg">
  <div className="space-y-2">
    ...
    <h4
      className="font-serif italic text-brand-text tracking-tighter leading-none"
  ...
  <div className="flex flex-col sm:flex-row gap-4">
    <button
      onClick={downloadPDF}
      className="flex-1 flex items-center justify-center gap-4 border-2 border-brand-border text-brand-text font-mono text-[10px] uppercase tracking-briefing font-bold py-6 hover:border-brand-accent hover:text-brand-accent transition-all group"
    >
    ...
    <button
      ...
      className="flex-1 flex items-center justify-center gap-4 bg-brand-accent text-white font-mono text-[10px] uppercase tracking-briefing font-bold py-6 hover:bg-brand-text transition-all group shadow-lg"
    >
      {recommendation.cta}
